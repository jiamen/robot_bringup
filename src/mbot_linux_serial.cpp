...
#include "../include/robot_bringup/mbot_linux_serial.h"

using namespace std;
using namespace boost::asio;        // IO口异步工作方式

// 串口相关对象
boost::asio::io_service io_sev;
boost::asio::serial_port sp(io_sev, "/dev/ttyUSB1");
...
void writeSpeed(double RobotV, double YawRate, unsigned char ctrlFlag)
{
    ...
    int i, length = 0;
    double r = RobotV / YawRate;        // mm， 求出来半径R    V = W * R
...
bool readSpeed(double& vx, double& vth, double& th, unsigned char& ctrlFlag)
{
    ...
    {
        ROS_ERROR("Recevived data check sum error!");
        return false;
    }
